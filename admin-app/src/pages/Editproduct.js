...
import { getAllSubcategories, getSubcategory } from '../features/subcategory/subcategorySlice';
import { getProduct, resetProductState, updateProduct } from '../features/product/productSlice';
...
const Editproduct = () => {
  ...
  const { loading, successMessage, error,product } = useSelector((state) => state.product); // Adjust slice name
  ...
  console.log(product)
    ...
    useEffect(() => {
      if (product) {
        ...
        const filePreviews = product.images.map((img) => `${image_url}${img.url}`);
