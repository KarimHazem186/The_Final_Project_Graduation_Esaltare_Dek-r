...
import { createCategory, resetCategoryState } from '../features/category/categorySlice';

const Newcategory = () => {
  const dispatch = useDispatch();
  const { loading, successMessage, error } = useSelector((state) => state.category);
  ...
  useEffect(() => {
    ...
    const formik = useFormik({
      ...
      const handleImageChange = (e) => {
        ...
        if (files.length + categoryData.images.length > 5) {
          toast.error("You can only upload up to 5 images.");
          return;
        }
      
        // Append the new images to the existing images
        const updatedImages = [...categoryData.images, ...files];
        setCategoryData((prev) => ({ ...prev, images: updatedImages }));
      ...
      const handleRemoveImage = (indexToRemove) => {
        const updatedImages = categoryData.images.filter((_, idx) => idx !== indexToRemove);
